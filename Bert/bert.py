...
class Bmm1(Module) :
    ...
    def forward(self, input1, input2) :
        return strided_bmm1(input1, input2)
...
class Bmm2(Module) :
    ...
    @torch.jit.ignore
    def forward(self, input1, input2) :
        return strided_bmm2(input1, input2)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        #seql = hidden_states.size(0)
        #seqs = hidden_states.size(1)
        #hidden_states = hidden_states.view(seql*seqs, hidden_states.size(2))
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
        #mixed_query_layer = mixed_query_layer.view(seql, seqs, self.all_head_size)
        #mixed_key_layer = mixed_key_layer.view(seql, seqs, self.all_head_size)
        #mixed_value_layer = mixed_value_layer.view(seql, seqs, self.all_head_size)
